...
var WS_SECRET = process.env.WS_SECRET || "vap-net-stats-has-a-secret";
...
function Node ()
{
	this.info = {
		...
		protocol: null,
		api: null,
		port: (process.env.LISTENING_PORT || 10801),
		os: os.platform(),
		os_v: os.release(),
...
Node.prototype.startWeb3Connection = function()
{
	...
	web3 = new Web3();
	web3.setProvider(new web3.providers.HttpProvider('http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '8575')));
...
Node.prototype.startSocketConnection = function()
{
	if( !this._socket )
	{
		...
		socket = new Socket( process.env.WS_SERVER || 'ws://localhost:3001' );
...
Node.prototype.getInfo = function()
{
	...
	try {
		this.info.coinbase = web3.vap.coinbase;
		this.info.node = web3.version.node;
		this.info.net = web3.version.network;
		this.info.protocol = web3.toDecimal(web3.version.vapory);
...
Node.prototype.getLatestBlock = function ()
{
	...
	if(this._web3)
	{
		...
		web3.vap.getBlock('latest', false, function(error, result) {
...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		...
		web3.vap.getBlockTransactionCount('pending', function (err, pending)
		{
			if (err) {
...
Node.prototype.getHistory = function (range)
{
	...
	{
		web3.vap.getBlock(number, false, callback);
	},
	function (err, results)
	{
		if (err) {
...
Node.prototype.setWatches = function()
{
	...
	web3.vap.isSyncing(function(error, sync) {
		if(!error) {
			if(sync === true) {
				web3.reset(true);
...
Node.prototype.setFilters = function()
{
	...
	this._latestQueue = async.queue(function (hash, callback)
	{
		...
		web3.vap.getBlock(hash, false, function (error, result)
		{
			self.validateLatestBlock(error, result, timeString);
	...
	try {
		this.chainFilter = web3.vap.filter('latest');
	...
	try {
		this.pendingFilter = web3.vap.filter('pending');
